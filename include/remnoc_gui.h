...
enum PICTURE_TYPE { NO_PICTURE, TILES }; // What's on screen?
...
class GUI_GRAPHICS {
 ...
 private:
    ...
    // here "tiles" took the role of "routers" from the NOC project;
    ...
    // tolor in which each tile and link should be drawn.
    vector<COLOR_TYPES> _tile_color;
    vector<int> _tile_occupancy; // how many IP/cores mapped to it?
    vector<bool> _tile_is_broken;
    vector<string> _tile_IP_name; // names of IP/cores mapped to each tile, if any;
